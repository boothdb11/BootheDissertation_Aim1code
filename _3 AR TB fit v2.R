...
simulate_incidence <- function(fore, pop.proj) {
  ...
  for (i in 1:length(fore$Boot)) {
    p <- pop.proj
    r <- fore$Boot[[i]]
    n <- rpois(nrow(r)*ncol(r), as.matrix(p*r))
...
summary_with_age <- function(inc, agp) {
  ...
  for (a in names(agp)) {
    ...
    n <- ifelse(length(ind)>1, rowSums(n.total[, ind]), sum(n.total[, ind]))
    pop <- ifelse(length(ind)>1, rowSums(p.total[, ind]), sum(p.total[,ind]))
    res[paste0(a, "N")] <- n
...
gnr <- ggplot(data=dat, aes(x=Year)) +
  ...
  scale_y_continuous("Incidence rate per 100,000", limits=c(0, 3.75)) +
